...
from data_wrangling import bag_of_words
...
def eval_model(vw_model, filepath, filename, labels, pred_filename):
    predictions = []
    ...
    if os.path.isfile(os.path.join(filepath, pred_filename)):
        os.remove(os.path.join(filepath, pred_filename))
    with open(os.path.join(filepath, pred_filename), "a") as pred_file:
...
def main():
    #  loading in data
    train_docs, test_docs, train_labels, test_labels = load_data(filepath)

    # data processing

    # store training data
    store_train_data_vw_format(
        filepath, filename=train_filename, docs=train_docs, labels=train_labels
    )
    # store testing data
    store_eval_data_vw_format(filepath, filename=test_filename, docs=test_docs)

    # generate vw_model with corresponding params
    vw_model = pyvw.vw(**vw_opts)

    # train vw_model
    train_model(vw_model, filepath, filename=train_filename)
