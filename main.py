...
def main():
    ...
    # pd_df has 3 sections for feature extraction:
    # 1) Subject
    # 2) pure_text (From and Subject dropped)
    # 3) text_data (Subject + pure_text)
    df = text_pre_processing(spacy_nlp, df, processed_data_fpath, pre_processed_filename,
                             load_file=True)

    # swap lemmatization with subword embeddings by bpemb
    df = text_lemmatization(spacy_nlp, df, ['pure_text', 'Subject'],
                             processed_data_fpath, pure_text_lemmatized_filename,
                             load_file=False)

    # creates new pd_series with key f"{pd_series}_unq"
    df = remove_dup_words(spacy_nlp, df, ['pure_text', 'text_data'], processed_data_fpath, dups_removed_filename,
                            load_file=False)
    ...
    # creates new pd_series with key f"{pd_series}_w_vect"
    ...
    df = subword_embedding(bpemb_en, df, ['text_data_unq'], processed_data_fpath, words_vectorized_filename,
                       load_file=False)
    ...
    pd_to_vw_fmt(pd_df=data_train,
                 ...
                 subword_vector_sections_to_convert=['text_data_unq_sw_vect'],
